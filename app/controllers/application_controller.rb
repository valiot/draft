class ApplicationController < ActionController::Base
  ...
  def redirect_decisions
    ...
    return redirect_to event_review_path unless @event && @event.teams.find_by(attendee: @attendee)
    # Existe el evento y ya hiciste checkin
    return redirect_to event_select_path(@event) unless @event && @attendee
    # Existe el evento y ya aceptace la invitation
    return redirect_to event_checkin_path(@event) unless @event && @invitation.accepted?
    # Existe el evento y la invitation esta pendiente
    return redirect_to invitations_path unless @event && @invitation.pending?
